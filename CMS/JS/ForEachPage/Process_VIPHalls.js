...
function OpenDialog_Ins_VIPHalls() {
    ...
    $('#PositionShowDialog').setTemplateURL('../../Templates/Ins_VIPHalls.htm');
...
// Info: Xu ly insert 
/*############################################################################*/
function Ins_VIPHalls() {
    $.ajax({
        url: "/Action/ProcessBackendAction.ashx?ActionObject=Halls&action=Ins",
        type: "POST",
        dataType: "json",
        ...
        success: function (data) {
            if ((data.status == "success") || (IsReloadRadgrid == 1)) {
                ...
                showMessageBox("Thêm VIPHalls lỗi: <font style='font-size:9px'>" + data.message + "</font>");
            }
...
function OpenDialog_Upd_VIPHalls(IDHalls) {
    var title = "";
    var width = 810;
    var height = 660;
    if ($("#PositionShowDialog").length <= 0) {
...
function Fill_Halls_Dialog_Upd(IDHalls) {
    ...
    if (IDHalls > 0) {
        jQuery.ajax({
            url: "/Action/ProcessBackendAction.ashx?ActionObject=Halls&action=Sel_ByCode&IDHalls={0}".format(IDHalls),
            type: "POST",
            dataType: "json",
            data: "",
            success: function (data, dataStatus) {

                $("#PositionShowDialog").setTemplateURL('../../Templates/Upd_VIPHalls.htm');
                $("#PositionShowDialog").setParam('NumLang', sys_NumLang); //sys_NumLang : trong file SystemConfig.js
                $("#PositionShowDialog").setParam('Lang', sys_Lang);
...
function Upd_Halls(IDHalls) {
    $.ajax({
        url: "/Action/ProcessBackendAction.ashx?ActionObject=Halls&action=Upd&IDHalls=" + IDHalls,
        type: "POST",
        dataType: "json",
...
function Del_VIPHalls(IDHalls) {

    $.ajax({
        url: "/Action/ProcessBackendAction.ashx?ActionObject=Halls&action=Del&IDHalls=" + IDHalls,
        type: "POST",
        dataType: "json",
        data: $("#frmUpd_Halls").serialize(),
        success: function (data) {
