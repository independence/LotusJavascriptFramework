                        ...
                        $('thead tr:first th', g.hDiv).each( //add cell
							function () {
							    ...
							    if (typeof row.cell == 'undefined') {
							        ...
							        // Authory: NgocBM
							        // Date : 29/4/2013
							        // Info : Them cot nut delete/edit
							        //=========================================================================================
							        //alert("@@" + row[p.colModel[idx].name] + "--" + p.colModel[idx]['display'] + "--" + idx);
							        //alert("@@" + p.colModel[idx]['HTMLCode']);

							        if (typeof (p.colModel[idx]['ExtendBtn']) !== 'undefined') {
							            if (p.colModel[idx]['ExtendBtn'] == "UPD_ROW") {
							                td.innerHTML = '<span class="edit" id="' + p.colModel[idx]['name'] + row[p.colModel[idx].name] + '" style="padding-left: 20px;" onclick="' + p.colModel[idx]['Onpress'] + "(" + row[p.colModel[idx].name] + ');"  > Edit </span>';
							            }
							            if (p.colModel[idx]['ExtendBtn'] == "DEL_ROW") {
							                td.innerHTML = '<span class="del" id="' + p.colModel[idx]['name'] + row[p.colModel[idx].name] + '" style="padding-left: 20px;" onclick="' + p.colModel[idx]['Onpress'] + "(" + row[p.colModel[idx].name] + ');"  > Del </span>';
							            }
							            if (p.colModel[idx]['ExtendBtn'] == "DIS_ROW") {
							                td.innerHTML = '<span class="dis" id="' + p.colModel[idx]['name'] + row[p.colModel[idx].name] + '" style="padding-left: 20px;" onclick="' + p.colModel[idx]['Onpress'] + "(" + row[p.colModel[idx].name] + ');"  > Dis </span>';
							            }
							            if (p.colModel[idx]['ExtendBtn'] == "IMG_ROW") {
							                if (row[p.colModel[idx].name] != null) {
							                    if (row[p.colModel[idx].name].length > 0) {
							                        td.innerHTML = '<img class="imagethumb"  alt="Porcupine Framework" height="' + p.colModel[idx]['heightImg'] + '" width="' + p.colModel[idx]['widthImg'] + '" src="' + p.colModel[idx]['path'] + row[p.colModel[idx].name] + '">';
							                    }
							                    else {
							                        td.innerHTML = '<img class="imagethumb"  alt="Porcupine Framework" height="' + p.colModel[idx]['heightImg'] + '" width="' + p.colModel[idx]['widthImg'] + '" src="' + p.colModel[idx]['path'] + 'NoImage.jpg' + '">';
							                    }
							                }
							                else {
							                    td.innerHTML = '<img class="imagethumb"  alt="Porcupine Framework" height="' + p.colModel[idx]['heightImg'] + '" width="' + p.colModel[idx]['widthImg'] + '" src="' + p.colModel[idx]['path'] + 'NoImage.jpg' + '">';
