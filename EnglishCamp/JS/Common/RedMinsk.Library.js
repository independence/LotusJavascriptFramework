...
function WrapSegPaging(numRow, RowPerPage, segPerPage, curentPage, naviId, _sFunc, _itemTitle, isShowTotal, textFirstPage, textEndPage,PageKey) {
    ...
    var CSS = "<style>a.more_pages {";
    CSS = CSS + "background: none repeat scroll 0 0 rgba(0, 0, 0, 0); ";
    CSS = CSS + "border: 0 none; color: #008EC4; font-weight: normal;padding: 0; height: 22px;line-height: 22px; text-decoration: none; }";
    
    CSS = CSS + ".Pagging {";
    CSS = CSS + "color: #8F8D8E;";
    CSS = CSS + "font-weight: bold;";
    CSS = CSS + "height: 22px;";
    CSS = CSS + "line-height: 22px;";
    CSS = CSS + "padding: 3px 5px;";
    CSS = CSS + "}";

    CSS = CSS + ".Pagging_selected {";
    CSS = CSS + "background: none repeat scroll 0 0 #BDBDBD;";
    CSS = CSS + "border-bottom: 3px solid #00AEEF;";
    ...
    CSS = CSS + ".Pagging:hover {";
     CSS = CSS + "background: none repeat scroll 0 0 #FFFFFF;";
     CSS = CSS + "border-bottom: 3px solid #00AEEF;";
     CSS = CSS + "}";
    CSS = CSS + "</style>";
    ...
    sHTMLNavi += '<div class="pages ms_pages">';

    if (curentPage > 1 && constAlpha > 0) sHTMLNavi += '<a href="#" class="more_pages" href="#' + PageKey + '1" onclick="WriteClientParameter(\'page\',1);' + _sFunc + '">' + textFirstPage + '</a></li>';
    if (constAlpha > 0) sHTMLNavi += '<a href="#" class="more_pages Pagging" href="#'+PageKey + (parseInt(curentPage) - 1) + '" onclick=" ' + _sFunc + '">«</a>';
    for (i = startIndex; i < endIndex; i++) {
        if (i == curentPage)
            sHTMLNavi += '<a href="#" class="Pagging_selected">' + i + '</a>';
        else {
            sHTMLNavi += '<a  class="Pagging" href="#'+PageKey + parseInt(i) + '" onclick="WriteClientParameter(\'' + PageKey + '\',' + parseInt(i) + ');' + _sFunc + '">' + i + '</a>';
        }
    }
    if (constAlpha < numSeg - 1 && endIndex < numPage) sHTMLNavi += '<a  class="more_pages Pagging" href="#' + PageKey + (parseInt(curentPage) + 1) + '" onclick="WriteClientParameter(\'' + PageKey + '\',' + (parseInt(curentPage) + 1) + ');' + _sFunc + '">»</a></li>';
    if (constAlpha < numSeg - 1 && curentPage < numPage && numPage > numSeg) sHTMLNavi += '<a  class="more_pages Pagging" href="#' + PageKey + parseInt(numPage) + '" onclick="WriteClientParameter(\'' + PageKey + '\',' + parseInt(numPage) + ');' + _sFunc + '">' + textEndPage + '</a></li>';
    sHTMLNavi += '<div class="clear"></div></div>';

    if (numRow <= 0) sHTMLNavi = '';
    {
        sHTMLNavi = CSS + sHTMLNavi;
        $(naviId).html(sHTMLNavi);
    }
...
function FillterPrice(_container, _PriceId) {
    if (typeof (_PriceId) == "undefined") var _PriceId = 0;
    jQuery.ajax({
        url: "/ActionHandler.ashx?ActionObject=objPriceGroup&action=getall_PriceGroupByType&Type=1",
        type: "POST",
        dataType: "json",
        data: "",
        success: function (data, dataStatus) {
            $(_container).append('<option value="0">[--- Select price ---]</option>');
            for (var i = 0; i < data.PriceGroup.length; i++) {
                if (data.PriceGroup[i].ID == _PriceId) {
                    $(_container).append('<option selected="selected" value="' + data.PriceGroup[i].ID + '">' + data.PriceGroup[i].Name + '</option>');
                }
                else {
                    $(_container).append('<option value="' + data.PriceGroup[i].ID + '">' + data.PriceGroup[i].Name + '</option>');
                }
            }
        },
        timeout: 30000,
